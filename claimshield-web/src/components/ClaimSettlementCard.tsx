    ...
    <div className="settlement-reorder-wrap">
      <section className="card card-tint-blue settlement-card settlement-order-2">
        <div className="claim360-card-header">
          <h2>
            <Calculator size={14} style={{ verticalAlign: '-2px', marginRight: '0.35rem' }} />
            Assessment Summary
          </h2>
      ...
      {damageItems.length > 0 && (
        <section className="card damage-items-table-card settlement-order-1">
          ...
          <div className="damage-table-wrap">
            <table className="damage-items-table">
              <thead>
                <tr>
                  <th>S NO</th>
                  <th>ITEM NAME</th>
                ...
                {damageItems.map((item, index) => {
                  ...
                  return (
                    <tr key={item.damageAssessmentItemId}>
                      <td className="damage-sno">{index + 1}</td>
                      <td className="damage-item-name">{item.componentName || '—'}</td>
                      <td>
                        <span
                          className={`damage-action ${
                            item.replacementRequired
                              ? 'damage-action-replace'
                              : 'damage-action-repair'
                          }`}
                        >
                          {item.replacementRequired ? 'Replace' : 'Repair'}
                        </span>
                      </td>
                      <td className="damage-category">{category}</td>
                      <td className="damage-money">{formatCurrency(item.labourAmount)}</td>
                      <td className="damage-money">{formatCurrency(item.partsAmount)}</td>
                      <td>
                        <input className="damage-number-input" type="number" min="0" step="0.01"
                          disabled={!canEditAmount} value={edit.dep}
                          onChange={(e) => handleItemFieldChange(item.damageAssessmentItemId, 'dep', e.target.value)} />
                      </td>
                      <td>
                        <input className="damage-number-input" type="number" min="0" step="0.01"
                          disabled={!canEditAmount} value={edit.rr}
                          onChange={(e) => handleItemFieldChange(item.damageAssessmentItemId, 'rr', e.target.value)} />
                      </td>
                      <td>
                        <input className="damage-number-input" type="number" min="0" step="0.01"
                          disabled={!canEditAmount} value={edit.td}
                          onChange={(e) => handleItemFieldChange(item.damageAssessmentItemId, 'td', e.target.value)} />
                      </td>
                      <td>
                        <input className="damage-number-input" type="number" min="0" step="0.01"
                          disabled={!canEditAmount} value={edit.ptg}
                          onChange={(e) => handleItemFieldChange(item.damageAssessmentItemId, 'ptg', e.target.value)} />
                      </td>
                      <td>
                        <input className="damage-number-input" type="number" min="0" step="0.01"
                          disabled={!canEditAmount} value={edit.others}
                          onChange={(e) => handleItemFieldChange(item.damageAssessmentItemId, 'others', e.target.value)} />
                      </td>
                    </tr>
                  )
                })}
              ...
              <tfoot>
                <tr>
                  <td colSpan={4} className="damage-total-label">Total</td>
                  <td className="damage-total-value">
                    {formatCurrency(damageItems.reduce((sum, item) => sum + (item.labourAmount ?? 0), 0))}
                  </td>
                  <td className="damage-total-value">
                    {formatCurrency(damageItems.reduce((sum, item) => sum + (item.partsAmount ?? 0), 0))}
                  </td>
                  <td className="damage-total-value">
                    {formatCurrency(Object.values(itemEdits).reduce((sum, edit) => sum + toNum(edit.dep), 0))}
                  </td>
                  <td className="damage-total-value">
                    {formatCurrency(Object.values(itemEdits).reduce((sum, edit) => sum + toNum(edit.rr), 0))}
                  </td>
                  <td className="damage-total-value">
                    {formatCurrency(Object.values(itemEdits).reduce((sum, edit) => sum + toNum(edit.td), 0))}
                  </td>
                  <td className="damage-total-value">
                    {formatCurrency(Object.values(itemEdits).reduce((sum, edit) => sum + toNum(edit.ptg), 0))}
                  </td>
                  <td className="damage-total-value">
                    {formatCurrency(Object.values(itemEdits).reduce((sum, edit) => sum + toNum(edit.others), 0))}
                  </td>
                </tr>
              </tfoot>
            </table>
          ...
          {canEditAmount && (
            <div className="damage-table-actions">
              <button type="button" onClick={() => void handleSaveItems()} disabled={savingItems}>
                ...
                {savingItems ? 'Saving…' : 'Save damage item figures'}
              </button>
            </div>
          )}
        </section>
